...
class Trainer:
    def __init__(self, height = 64, width = 64, epochs = 50000, batch = 32, checkpoint = 50, train_data_path_A = '',train_data_path_B = '',test_data_path_A='',test_data_path_B='',lambda_cycle=10.0,lambda_id=1.0):
        ...
        self.X_train_B, self.H_B, self.W_B, self.C_B  = self.load_data(train_data_path_B)#,graycolor=True
    ...
    def load_data(self,data_path,amount_of_data = 1.0,graycolor=False):
    ...
    def train(self):
        #i=np.random.randint(309)
        #discriminator_loss_A=""
        #generator_loss=""
        i=0
        #report=list()
        #report.add(["Epoch:","Discriminator_A :: Loss:","Generator :: Loss:"])
        for e in range(self.EPOCHS):
            ...
            while min(len(X_train_A_temp),len(X_train_B_temp))>self.BATCH:
                ...
                #introduce label noise into the training process with the development of the batches for training the individual discriminators:
                ...
                if b % self.CHECKPOINT == 0 :
                    label = str(e)+'_'+str(b)
                    self.plot_checkpoint(label,discriminator_loss_A,generator_loss)
                    print ('Epoch: '+str(int(e))+', [Discriminator_A :: Loss: '+str(discriminator_loss_A)+'], [ Generator :: Loss: '+str(generator_loss)+']')
            #report.add(str(int(e)),str(discriminator_loss_A),str(generator_loss))    
    ...
    def plot_checkpoint(self,b,discriminator_loss_A,generator_loss):

        row='Epoch: '+str(b)+', Discriminator_A :: Loss: '+str(discriminator_loss_A)+', Generator :: Loss: '+str(generator_loss)+'\n'
        with open('../data/evolucion.csv', 'a', newline='') as file:
            file.write(row)

        orig_filename = "../data/batch_check_"+str(b)+"_original.jpg"

        image_A = self.X_test_A[0]
        ...
        for i in range(3):
            ...
            if i!=0:
                Image.fromarray(images[i]).save("../data/file_"+str(b)+"_"+str(i)+".jpg")
            axes[i].imshow(image)
            axes[i].set_title(titles[i])
            axes[i].axis('off')
        fig.savefig("../data/batch_check_"+str(b)+".jpg")
        plt.close('all')
        return
